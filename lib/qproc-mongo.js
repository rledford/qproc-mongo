      ...
      for (const k in options.fields[f]) {
        switch (k) {
          case 'alias':
            ...
            break;
          case 'default':
            if (typeof options.fields[f][k] === 'object') {
              throw new Error(`Invalid default value for field [ ${f} ]`);
            }
  ...
  return {
    ...
    exec: function(q) {
      ...
      for (let k in q) {
        ...
        const resolvedType = fields[helpers.resolveField(k, fields)];
        ...
        if (resolvedType) {
          ...
          if (isNin || isAll) {
            ...
            value = list.map(function(v) {
              return helpers.convertStringToType(v, resolvedType);
            });

            Object.assign(qproc.filter[k], { [operator]: value });
          } else if (regex.exec(q[k])) {
            if (resolvedType === types.String) {
              let exp, match, flags;
            ...
            q[k].split(',').forEach(function(range) {
              ...
              if (Array.isArray(value)) {
                value = value.map(function(v) {
                  return helpers.convertStringToType(value, resolvedType);
                });
              } else {
                value = helpers.convertStringToType(value, resolvedType);
              }
