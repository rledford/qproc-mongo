...
module.exports = {
  createProcessor: function(options) {
    ...
    return function(req, res, next) {
      ...
      for (let k in alias) {
        if (req.query[k] && !req.query[alias[k]]) {
          req.query[alias[k]] = req.query[k];
        }
      }
        ...
        if (fields[k]) {
          ...
          if (nin.exec(req.query[k])) {
            ...
            if (fields[k] === types.String) {
              ...
              exp = req.query[k].substring(regex.lastIndex);
          ...
          if (!Object.keys(req.qproc.filter[k]).length) {
            delete req.qproc.filter[k];
          }
